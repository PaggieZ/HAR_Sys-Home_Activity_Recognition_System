...
def get_newRow_from_line(line):
    ...
    sensorValue = [cols[3]]
    activity = [cols[4]]
    newRow = pd.DataFrame({"timestamp": timestamp, "sensorID": sensorID, "newSensorID": newSensorID, "sensorValue": sensorValue, "activity": activity})
    return newRow
...
def count_bath_features(sampleData, threshold_min=1, max_temp=37, threshold_light=5):
    ...
    # Apply rolling max filter to sleep-labeled sensor readings 
    resampled_df = preprocessor.upsample_activity(activity='Sleep')
    # resampled_df.to_csv('output.csv', mode = 'w')
    # plotData.plot_AL_sleep(resampled_df)
    if resampled_df is None:
        ...
        filtered_df = preprocessor.apply_filters_activity(resampled_df, "sensorValue", activity='Sleep')
        sleepTime, wakeTime = preprocessor.get_AL_sleep_wake_time(df=resampled_df)
        try:
    ...
    # Apply rolling max filter to toilet-labeled sensor readings
    resampled_df = preprocessor.upsample_activity(activity='Toilet')
    if resampled_df is None: 
        return None, None
    filtered_df = preprocessor.apply_filters_activity(resampled_df, "sensorValue")
    
    # Count bathroom trips and durations 
    Bathroom_State_Tracker = ToiletALTracker(IDLE, nighttime_hr=sleepTime_hr, nighttime_min=sleepTime_min, 
    ...
    motion_toilet_count = len(sampleData[(sampleData['sensorID'].str.contains('Bathroom')) & 
                                         ...
                                         (sampleData['activity'].str.contains('Toilet')) &
